function [sysName,netName] = gensim2(net,varargin)
    ...
    % New System
    sysName = createNewSystem(sysName);
    open_system(sysName);
...
function genNetwork(net,networkY,netName,sysName,st,inputNames,outputNames)
    ...
    % Remove trailing '|' from maskPromptString, so as not to create a
    % redundant dialog box
    if ~isempty(maskPromptString) && strcmp( maskPromptString(end), '|')
        maskPromptString(end) = [];
    end
...
function mask = transformMask
    ...
    %======================================================================
function name = createNewSystem(name)
    % createNewSystem   Create a new system with the given name
    %
    % If `name` is empty then a name is assigned.
    
    system = new_system( name, 'model' );
    % Because `name` might be empty we need to get the name to ensure that
